...
const onFinish = async (values: FieldType) => {
  ...
  try {
    const response = await fetch("https://ecommerce-api-r62c.onrender.com/dj-rest-auth/login/", {
      method: "POST",
      headers: {
...
const SignIn: React.FC = () => {
  ...
  const handleLoginSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // loginis logika
    // Call the onFinish function with the form values
    await onFinish({ username: email, password, remember: false });
  };
        ...
        <div className="login-main">
          <div className="login">
            ...
            <form onSubmit={handleLoginSubmit}>
              ...
              <div className="emailerror-div">{!isEmailValid && formSubmitted && <span className="emailerror">Incorrect Email!</span>}</div>

              <div className="login-input-box">
                <input type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={handlePasswordChange} />
                <img src={showPassword ? passwordOpenImg : passwordImg} alt="" onClick={togglePasswordVisibility} className="password-toggle" />
              </div>

              <div className="passworderror-div">{formSubmitted && <span className="passworderror">Incorrect password!</span>}</div>
              ...
              <button type="submit" className="btn" onClick={(event) => handleLoginButtonClick("", event)}>
                Sign In
              </button>
